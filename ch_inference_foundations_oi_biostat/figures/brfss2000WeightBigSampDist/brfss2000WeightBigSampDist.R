...
for (i in 1:N) {
  tmp <- sample(nrow(brfss.2000.adults), 200)
  tmp <- tmp[!is.na(tmp)][1:100]
  means[i] <- mean(brfss.2000.adults$weight[tmp], na.rm = TRUE)
  setTxtProgressBar(pb, i)
}

myPDF('brfss2000WeightBigSampDist.pdf', 8, 3.15,
      mar = c(3.3, 4, 1.4, 1),
      mgp = c(2.7,0.55,0))
layout(matrix(1:2, 1),
       c(4.3, 3))
...
s <- sd(brfss.2000.adults$weight, na.rm = TRUE) / 10
rect(m - s / 100, 0,
     m + s / 100, 1350,
     col = '#00000044',
     border = '#00000000')
