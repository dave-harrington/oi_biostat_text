...
The first column labels the $4$ profiles, and columns $2-3$ show the values predictors.  The remaining columns contain the following data for each profile:
\begin{itemize}
...
The residual is small because the predicted value $45.02$ is close to the observed number of responses $44$.
 ...
 A $\chi^2$ goodness of fit is based on $\sum_i r_i^2$, with degrees of freedom equal to the number of profiles minus the total number of parameters (including the intercept).  For the TB data, the $\chi^2$ statistic $X^2$ is
\begin{align*}
...
\begin{align*}
  \text{TPR} &= P(\hat{y} = 1| \text{hyperuricemia present}) \\
     ...
     &= 1 - \text{FNR},
\end{align*}
$\text{TPR} = 0.60$ corresponds to $\text{FNR} = 0.40$, and the point $(0.40, 0.60)$ (approximately) corresponds to the threshold value $0.20$ in Figure~\ref{figure:fprFNRByCutoffHuBMISex}.
...
The 45-degree line on an ROC plot is used to distinguish between tests which may have some value vs. those which are worse than guessing.  Let $\text{D}$ and $\overline{\text{D}}$ indicate the presence or absence of a disease D respectively, and $+$ and $-$ indicate a positive or negative test.  For points on the 45-degree line, sensitivity equals 1 - specificity, so 
\begin{align*}
P(+ | \text{D}) &= 1 - P(- | \overline{\text{D}}) \\
  &= P(+ | \overline{\text{D}}).
\end{align*}
...
\begin{align*}
   \frac{P(\text{D}| +)}{P(\overline{\text{D}} | +)} =
      \frac{P(D)}{P(\overline{\text{D}})}.
\end{align*}
...
\begin{align*}
\text{AUC} = \frac{P(+ | \text{D})}{P(+| \overline{\text{D}})}.
\end{align*}
