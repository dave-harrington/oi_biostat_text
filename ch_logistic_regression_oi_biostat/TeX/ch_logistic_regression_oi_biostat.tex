...
The red curve, which shows a model-based estimate of the probability of hyperuricemia using logistic regression, is a better fit to the data across the range of BMI values. The logistic regression model predicts the log odds of hyperuricemia, rather than the probability of hyperuricemia.\footnote{Note that "log" refers to the natural log.} Suppose $E$ is an event, $x$ is a predictor, and $p_{E}(x)$ is the conditional probability of $E$ given $x$. The odds of $E$ given $x$ are $\frac{p_{E}(x)}{1 - p_{E}(x)}$. 
...
Figure~\ref{figure:probVsObsHuBMI} shows values of the outcome variable $Y = 0$ (no hyperuricemia) or $Y = 1$ (hyperuricemia) plotted against model predicted probabilities. It is the analogue of plotting observed versus predicted values in linear regression, but because all the observed values are clustered at 0 or 1, it is less useful as a diagnostic than in linear regression.  As noted earlier, close inspection of the plot indicates that larger predicted probabilities tend to have a higher frequency of $Y = 1$, but the trend is subtle.
...
These data suggest that with 95\% confidence, an increase of 1 unit BMI is associated with a higher odds of hyperuricemia by a factor of 1.1 to 1.3. In other words, with 95\% confidence, when comparing individuals who differ in BMI by 1 unit, the individual with higher BMI has a higher odds of hyperuricemia that may be from 1.1 times as large to 1.3 times as large relative to the individual with lower BMI.
...
Since the coefficients for both predictors are statistically significant, it is reasonable to expect that model including \var{sex} and \var{bmi} is significantly better than a model with only an intercept, and the deviance statistic confirms that.  The software \textsf{R} reports that the null and residual deviances are 486.22 and 455.27, respectively.  The difference, 30.95, yields $p < 0.001$ from a $\chi^2$ with 2 degrees of freedom.  
...
\end{align}
In a model that includes BMI, the OR for hyperuricemia comparing females to males with identical values of BMI is $e^{-0.480} = 0.612$, which differs slightly from the earlier estimate of $0.502$ from the model including only \var{sex}.  Confidence intervals are calculated using standard errors just as in single variable logistic regression.


\begin{exercisewrap}
  \begin{nexercise}
    Calculate a 95\% confidence intervals for the odds ratio of hyperuricemia associated with a three unit increase in BMI for two individuals of the same sex.     \footnotemark{}
  \end{nexercise}
\end{exercisewrap}
...
The $p$-value for the interaction term is $0.544$, so the evidence against the null hypothesis $\beta_3 = 0$ is weak. The observed difference in association between the log odds of hyperuricemia and BMI between males and females is consistent with what would be expected (due to sampling variability) if there were actually no population-level difference in association. In this model, there is no support for the claim that the relationship between hyperuricemia and BMI differs by sex.  Exercise~\ref{exercise:BMISexInteractionModel} explores the interpretation of a model with an interaction term.
...
This chapter avoids the use of the terms effect modifier and effect modification in observational studies. The term "effect" implies a causal link that cannot be established in an observational study with the methods described in this text.

\begin{comment}
 and "effect modification" connotes an underlying scientific relationship that may not be accurately modeled in a typical regression model with interaction terms.  Fitting a model with interactions is a statistical device to improve the fit of a model and that device may not match the underlying phenomenon.
\end{comment}

It is very common in applications to label the non-interaction terms as \term{main effects} and interaction terms as \term{interaction effects}.  The terminology can be a useful abbreviation as long as no causal association is meant.
...
The major weakness here: does not make the point that model checking is for explanatory models fitting a population (inference); predictive accuracy used when the model will be used to predict individual outcomes.  Important to make the case of the importance of checking out of sample accuracy.  Make the case that checking accuracy in study sample relies on strong assumption that population is perfectly representative.

Be more specific in the summary.  Perhaps list all the methods at the beginning with settings where they are used.

Stages in checking: asess fit, in sample prediction error, out of sample prediction error.  Important to give a context.  Explain that methods are more complex than linear, give specific reference to linear regression diagnostics.  Material can be consisted optional for most readers.

Reasonable to make the case that model checking here has important similarities and differences when compared to linear regression.

The section on ROC curves is particularly week.  Best to fix it when the overal picture is more clear.

\end{comment}

There are two general approaches to checking the fit of a logistic regression, one based on goodness-of-fit statistics and another on statistics that estimate the accuracy of predictions. The two approaches serve different purposes.  When the primary goal of an analysis is drawing inferential conclusions about the relationship between an outcome variable and predictors, goodness-of-fit statistics should be used to check whether the model fits the data reasonably well.  A reasonable model that does not substantially deviate from the model assumptions is usually sufficient for inference about parameters. The explanatory model fit to assess the association between hyperuricemia and the predictors BMI and sex is an example of an analysis conducted to draw inference about parameters.

Predictive accuracy of a model should be checked when the purpose of an analysis is to predict individual outcomes or classify individuals according to predicted outcomes for individuals not in the original study population.  The case study in the next section explores models for classifying patients admitted to a hospital emergency department according to the risk of death within 30 days of admission (30-day mortality).  In this analysis, the main goal is to assess the likelihood of classification errors arising from inaccurate predictions of 30-day mortality, rather than understand what variables might be strongly associated with higher risk of 30-day mortality.

Some model diagnostics, such as calibration plots, are useful both for checking the fit of a model and predictive accuracy.  Regardless of whether one is examining goodness-of-fit or predictive accuracy, probabilities calculated from an estimated  model are referred to as predicted probabilities.  Predicted numbers of events are sometimes also called expected number of events.
...
How much better than random guessing is a prediction rule for hyperuricemia based on BMI and sex? The AUC for Figure~\ref{figure:ROCHuBMISex} is 0.678 -- the model has a $68\%$ chance of correctly distinguishing between an individual with versus  without hyperuricemia.  There is no single definition of a good AUC, but are guidelines that may be useful in some settings.  For biomedical data, Hosmer, et.\ al.\footfullcite{hosmer2013applied}, recommend the following:
\begin{figure}[ht]
\centering
  \begin{tabular}{cl}
  \hline \\
...
Figure~\ref{figure:ROCHuBMISex} shows four example ROC curves corresponding to hypothetical models with increasing ability to discriminate: the 45-degree line, AUC = 0.5, random guessing; the green curve, AUC = 0.667, poor discrimination; blue curve, AUC = 0.785, acceptable discrimination; red curve, AUC = 0.874, excellent discrimination. ROC curves will be used to compare models for triage in an emergency department in the case study in Section~\ref{caseStudy}.
...
The full data for both cohorts are contained the data package \texttt{oibiostat}, as \texttt{DanishEDPrimaryCohort} and \texttt{DanishEDValidationCohort}.  These datasets have also been posted by the authors (\url{DOI:10.5061/dryad.m2bq5}).  The datasets in \texttt{oibiostat} have been formatted for readability.
...
\subsection{Modeling the relationship between 30-day mortality and DEPT triage score, age and sex. The named colors on the horizontal axis correspond to DEPT codes.}
