...
The solutions have to be inserted in the common file eoce_solutions.tex 
when the chapter is moved to git hub.  The file eoce_sols_temp.tex in the draft text directory holds the draft solutions.  This file can be used to check compilation.

When we are satisfied with the exercises and solutions both should be copied into the 
question bank.

Possible additional exercises
  - Compare models using AIC, calculated from deviance statistics.
...
\eoce{\qt{Hyperuricemia and BMI} The fourth quintile of BMI in 
...
\eoce{\qt{Logistic Regression short answer, I} 

For the true/false questions, provide a reason for you answer.  The short answer questions can usually be answered in 2 - 3 sentences.
...
\eoce{\qt{Logistic Regression short answer, II} 
...
\item  Suppose the dataset was gathered in a prospective study with exposure based sampling.  Is the information part~(b) sufficient to estimate the probability of the outcome, given a value of the exposure variable?
...
\eoce{\qt{TB treatment interruption and secondary education}

% no  solution yet in draft sol folder

The two-way table in Figure~\ref{figure:tbInterruptionEduExample} summarizes the data
used to estimate the model in Figure~\ref{figure:tbInterruptionEduLogReg} in Example~\ref{example:tbInterruptionEducation}.
...
\eoce{\qt{Prevalence differences and ratios from logistic regression.
\label{exercise:prevDifferencesRatiosMalesFemales}}

\begin{parts}

\item Calculate the estimated prevalence of hyperuricemia for a female with BMI~28 and for one with BMI~30 used in Example~\ref{example:probsDependOnOtherVars}.

\item Calculate the estimated prevalence difference and ratio for the same two females.
...
\eoce{\qt{HIV test status and TB treatment interruption. \label{exerise:HIVTestStatusInterruption}}

Show that the conditions for a $\chi^2$ test are met for the data displayed in Figure~\ref{figure:hivInterruption}.
...
\eoce{\qt{Emergency room outcomes in Denmark, Part I.
...
\eoce{\qt{Emergency room outcomes in Denmark, Part II.
...
\eoce{\qt{Emergency room outcomes in Denmark, Part III.
...
\item The residual deviance for the model in Figures~\ref{figure:mort30TriageAgeSexSample1000} and \ref{figure:mortTriage1000} are, respectively, $407.58$ and $463.6$.  Conduct a test of the hypothesis that the pair of variables \var{age} and \var{sex} do not any information to a model based on triage score alone.

\item Does the AIC increase or decrease if \var{age} and \var{sex} are added to the model with triage?

\item Based on your answer to the above, do you believe that both age and sex should be retained in the model?  Explain your answer.

\end{parts}

}{}

\eoce{\qt{Interaction in logistic regression, Part I.
...
\eoce{\qt{Interaction in logistic regression, Part II.
...
\item Use the information in Figures~\ref{figure:bmiHyperuricemiaFemalesLogReg} and \ref{figure:bmiHyperuricemiaMalesLogReg} to calculate the two variances of the log odds estimates of BMI for females and males. 
...
\eoce{\qt{Smoking and marijuana use, Part I.
   \label{exercise:smokingMarij}}

Figure~\ref{} shows the association between smoking at least 100 cigarettes a year and regular marijuana use and sample of 500 participants from the NHANES data in the package \texttt{oibiostat}.  Figure~\ref{} below shows a logistic model fit to the same data, with response regular marijuana use and the smoking variable (\texttt{Smoke100}) as the predictor.
...
\item Show that a 95\% confidence interval for the log(OR) comparing smokers to non-smokers calculated from the logistic regression is approximately equal to the interval calculated from the contingency table using the formula in Section~\ref{figure:marijSmokeAgeGenderWorklogreg}.
...
\eoce{\qt{Smoking and marijuana use, Part II.
