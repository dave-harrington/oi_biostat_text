...
m = glm(mort30 ~ triage + age + sex + age*sex, data = d,
        family = "binomial")

p  <- predict(m, type = "response")

lo.fit = predict(loess(d$mort30 ~ p, family = "gaussian", span = 2/3))
...
ns1 <- 4
myPDF("predVsObsMort30CalibrateAgeLT100DanishED.pdf", 7.7, 4.4,
      mar = c(3.9, 7, 0.5, 0.2),
      mgp = c(2.4, 0.55, 0))
noise <- rnorm(nrow(d), sd = 0.08)
plot(p, d$mort30 + noise / 5,
     type = "n",
     xlim = 0:1,
     ylim = c(-0.07, 1.07),
     axes = FALSE,
