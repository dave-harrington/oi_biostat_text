...
% prevalence differences and ratios, odd problem

\eocesol{
	Prevalence ratios and differences. Solution needed
}

\begin{comment}
% even problem, HIV test status and TB int
\eocesol{
HIV test status and TB int, need solution
}
\end{comment}

% odd problem, female horseshoe crabs
\eocesol{
female horse shoe crabs, needs solution
}{}

\begin{comment}
% even problem, emergency room outcomes, I
\eocesol{Emergency room outcomes in Denmark, I.  Need solution.
}{}
\end{comment}

% odd problem, emergency room outcomes, II
\eocesol{Emergency room outcomes in Denmark, II.  Needs solution.
}
...
(c)~Using the model in Figure~\ref{huBmiSexLogReg}, the estimated OR for hyperuricemia for BMI = 33.2 versus 30 $\text{kg/m}^2$ is $\exp[(3.2)(0.171)] = 1.728$, a value that is different from the two ORs calculated above.
}{}
