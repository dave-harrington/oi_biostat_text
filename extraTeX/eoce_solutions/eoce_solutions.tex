...
% solutions to all problems in dropbox file eoce_logisitic_sols.tex
% solutions to even problems only in that file

% 1  odds and probabilities, odd problem
...
% Diabetes, even problem


% Hyperuricemia and BMI odd problem,  not a great problem; eliminate? 
...
% Interpreting model parameters I, even problem


% Interpreting model parameters II, odd problem
...
(c) The odds ratio does not depend on the intercept, but the probabilities and hence the relative risk does.
 ...
 % CPR and survival to discharge, I, even problem

% CPR and survival to discharge, II, odd problem part (d) correct?
...
% Hyperuricemia and dietary magnesium, I, even problem

% Hyperuricemia and Age, odd problem
...
% Logistic regression short answer, I, even problem


% Logistic regression short answer, II
...
% TB treatment interruption and secondary education, even problem
% needs solution

% Hyperuricemia and dietary magnesium, II, odd problem
...
% Hyperuricemia and age, II} even problem

\begin{comment}

\eocesol{
  (a)~The $z$ statistic is the estimate divided by its standard error, $-0.007/0.015 = -0.467$.
(b)~No, the data do not show a statistically significant association, since $p = 2P(Z < -0.467) = 0.641$.
(c)~A $95\%$ confidence interval for the estimate is $-0.007 \pm (1.96)(0.015) = (-0.036, 0.022)$.   With 95\% confidence, a 1 year change in age is associated with a change in log odds from $-0.036$ to $0.022$.
(d)~First construct the confidence interval on the log odds scale.  The estimated model coefficient $b_1$ is the change in log odds corresponding to a one year change in age.  When age increases from $50$ to $75$, the change in log odds will be $(75 - 50)b_1 = (25)(-0.007) = -0.175$.  On the log odds scale, the $95\%$ interval for the change will be the confidence interval for $25b_1$.  Since the standard error of $25b_1 = (25)(0.015) = 0.375$, the $95\%$ interval for the change in log odds is  $-0.175 \pm 1.96 (0.375) = (-0.910, 1.175)$.  With $95\%$ confidence the odds ratio is between $ \exp(-0.910), \exp(0.560) = (0.403, 1.751)$.  The confidence interval on the log odds scale could also have been calculated by multiplying the upper and lower bounds for the confidence interval for $b_1$ by 25.
}

\end{comment}

% Radon and leukemia, odd problem
...
% Challenger disaster, II. Even problem
% figure is in logistic regression draft folder under figures,  until I figure out where it should go

\begin{comment}

\eocesol{
% solution from OI Q Bank 
%\begin{parts}
(a)~The calculation of the probabilities are as follows:
\begin{align*}
log\left(\frac{\hat{p}_{51}}{1 - \hat{p}_{51}}\right) 
= 11.6630 - 0.2162 \times 51 
&\longrightarrow \hat{p}_{51} 
= \frac{e^{11.6630 - 0.2162 \times 51}}{1 + e^{11.6630 - 0.2162 \times 51}} 
= 0.654 \\
log\left(\frac{\hat{p}_{53}}{1 - \hat{p}_{53}}\right) 
= 11.6630 - 0.2162 \times 53 &\longrightarrow \hat{p}_{53} 
= \frac{e^{11.6630 - 0.2162 \times 53}}{1 + e^{11.6630 - 0.2162 \times 53}} 
= 0.551 \\
log\left(\frac{\hat{p}_{55}}{1 - \hat{p}}_{55}\right) 
= 11.6630 - 0.2162 \times 55 
&\longrightarrow \hat{p}_{55} 
= \frac{e^{11.6630 - 0.2162 \times 55}}{1 + e^{11.6630 - 0.2162 \times 55}} 
= 0.443
\end{align*}
(b)~The plot is shown below:
\begin{center}
\includegraphics[width=0.6\textwidth]{figures/challenger_disaster_pred_damage_temp.pdf} 
\end{center}
(c)~ For logistic regression to be appropriate in this case, each O-ring must 
be independent of the others. We must assume independence is true (or very 
nearly so) if we are to believe the model. For example, if the O-ring 
manufacturing process has changed dramatically over the time of the different 
shuttle launches, then we should be skeptical of independence and therefore 
also of the model.
}{}

\end{comment}

%  Fracture study, odd problem
...
\begin{comment}
% even problem, HIV test status and TB int
\eocesol{
The conditions for the $\chi^2$ test are discussed in Section~\ref{theChiSquaredTestStatistic}. The formulas for expected cell counts under the assumption of independence are given in Section~\ref{twoWayTablesExpectedCounts}.  The validity if of a $\chi^2$ in a $2\times2$ table are both met in this example.  The data from the participating subjects can is independent between subjects, and all the expected cell counts are at least 10.  The smallest observed cell count is 52, and the expected count for that cell is ($127 \times 719)/1233 = 74.06$.  The other expected cell counts are all at least 10.  
}
\end{comment}

% odd problem, female horseshoe crabs
\eocesol{
...
\begin{comment}
% even problem, emergency room outcomes, I
\eocesol{
(a)~The reference category is "red".
(b)~Yes, the pattern is consistent with the original coding.  Because the log(OR) is negative for each predictor, each color is associated with a lower OR than "red", the category denoted as life-threatening.  Moreover, the descreasing pattern of the predicted log(OR)s (compared to the color red) is consistent with lower risk as the color coding changes from "orange" to "green".
(c)~The algebraic form of the model is
\begin{align}
\log\left[\text{odds}_{\text{mortality}}(\text{triage score})\right] = -1.551 -0.958\texttt{triageorange}
 - 1.290\texttt{triageyelow}  - 1.585\texttt{triagegreen}.
 (d)~Yes.  In this model -1.551 is the log(odds) of 30-day mortality among participants coded "red".  The odds of 30-day mortality is $\exp(-1.551) = 0.212$.  
 (e)~The estimated OR is $\exp(-1.290) = 0.275$.  The 95\% confidence interval for the log(OR) is $-1.290 \pm (1.95)(0.468) = (-2.207,-0.373)$.  The confidence interval for the OR is $(\exp(-2.207, -0.373) = (0.110, 0.689).
 (f)~The log(OR) comparing "yellow" to "orange" is the difference of the two log(OR)s, $-1.290 - (-1.585) = 0.295$, so $\text{OR} = exp(0.295) = 1.343$.  The odds of 30-day mortality for a participant coded "yellow" are approximately 34\% larger than for someone coded "green".
\end{align}
}{}
\end{comment}
% odd problem, emergency room outcomes, II
\eocesol{
(a)~The estimate of the intercept is $log(7/33) = -1.551$.
(b)~The estimate for the triage category "green" is the log of the estimated OR comparing "green" to "red", $\text(log((11/253)/(7/33)) = -1.585$.
...
\begin{comment}
% even problem, emergency room outcomes, III
\eocesol{
(a)~No.  The intercept would correspond to the log(odds) of death within 30 days for a participant with all predictors at baseline value, so age would have value 0.
(b)~Increased risk. 
(c)~The observed difference between the residual variances is 56.02.  The difference has a $\chi^2$ distribution with $5 - 3 = 2$ degrees of freedom so is highly significant.
(d)  While the pair of variables \texttt{age} and \texttt{sex} is important, the model suggests that \texttt{sex} adds little information.  Before adding both variables a deviance statistic should be used to check the whether \texttt{sex} is useful when added to a mode with color score and age. 
}{}
\end{comment}

% odd problem, interaction in regression I
...
% even problem, interaction in regression
\begin{comment}
\eocesol{
(a)~The two variances are $0.059^2 = 0.0035$ (females) and $0.049^2 = 0.0024$ (males).
(b)~Since the two estimates are based on separate and independent groups, the variance of the difference of the estimates is the sum of the variances, $0.0059$.  The standard deviation of the difference is $\sqrt(0.0059) = 0.077$.  This is the standard error of the estimate of the difference.
(c)~A test of the null hypothesis that the difference of the population parameters is 0 can be based on the difference divided by its standard error, $(0.198 - 0.152)/0.077 = 0.594$.  Under the assumption that the individual estimates are approximately normally distributed, the difference will have a normal distribution with mean 0 and standard error $0.077$.  Since the standardized difference is less than 1.96, the difference is not significantly different from 0.
