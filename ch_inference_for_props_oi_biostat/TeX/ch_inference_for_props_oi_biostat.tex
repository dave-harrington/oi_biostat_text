...
Confidence intervals and tests for risk differences use the methods for comparing two binomial proportions outlined in Section~\ref{differenceOfTwoProportions}. When the conditions described in Section~\ref{samplingDistributionDiffTwoProps} are met, a 95\% confidence interval for the difference $p_1 - p_2$ of two proportions is given by
\begin{align*}
...
\begin{onebox}{Confidence intervals for log relative risk}
...
\begin{comment}
%JV: can't find this reference anymore

The concept of odds was widely used in gambling before medical research, and the use of odds to set fair return on a bet is discussed in Section~\ref{notesTwoSamplesBinaryData}.
\end{comment}
...
%should we add explicit checking of the conditions for the test?  If so, we should do it in the previous examples as well.  Perhaps that would make a good inline or end of chapter exercise.

%DH: yes, we should be more explicit about this.  The expected cell counts in the first column are 13.18 and 65.81, so the conditions are met.  Let's put it in an exercise

%JV: perhaps better in the eoce, since assumptions checking is discussed in Ch 8


%JV: Can risk be used below? How were the data sampled?
%DH:  I don't think risk can be estimated.  It is not completely clear how the data were collected, but it seems highly likely that accidents were either sampled or all accidents used, according to a fatality or not, and seat belt use was retrospectively coded.
...
% decided not to expand on the above because it is a complicated issue; the failure to reject often enough  under the null decreases power.
...
A well-known example from the history of statistics, described by Fisher in 1935\footfullcite{fisher:1935} illustrates the difficulty.  Fisher tested the claim that a colleague (Dr. Muriel Bristol) could distinguish whether milk or tea had been added to a cup of tea first.  Fisher asked the colleague to test 8 cups of tea, four of which had milk added first and four had tea added first.  The data from the experiment are shown in Figure~\ref{figure:fisherTeaTasting}

\begin{figure}[h]
	\centering
	\begin{tabular}{ll rrr r}
		\hline
        & & \multicolumn{2}{c}{\textbf{Guess poured first}} &  \\
        & \hspace{2mm} & Milk & Tea &
        \hspace{2mm} & Total \\
        \textbf{Actual poured first} & & & & & \\
        \hline
        Milk & & 3 & 1 & & 4    \\
        Tea &	& 1 & 3 & & 4    \\
        Total & & 4 & 4 & & 8 \\
		\hline
	\end{tabular}
	\caption{Outcome in Fisher's tea tasting experiment}
    \label{figure:fisherTeaTasting}
\end{figure}	

A test of the hypothesis that Bristol cannot tell the difference is a test $H_0:\text{OR} = 1$ against either a one- or two-sided alternative.  For the one-sided alternative $H_1: \text{OR} > 1$ (favoring Bristol's claim), the $p$-value for the test is the sum of the probabilities of tables that are as or more extreme than the one observed: the two tables with either a 3 or 4 in the upper right cell. Conditional on the row and column totals, there are $\binom{8}{4}$ total tables that could describe the outcome. A 3 in the upper left cell arises when Bristol chooses 3 of the 4 choices correctly and 1 of the 4 incorrectly. The value 3 in the upper left has conditional probability
\[
  \frac{\binom{4}{3} \binom{4}{1}} {\binom{8}{4}} = 0.229.
\]
Using a similar argument the more extreme table with a 4 in the upper left cell has probability
\[
  \frac{\binom{4}{4} \binom{4}{0}} {\binom{8}{4}} = 0.014.
\]
If Bristol guessed 3 out of 4 correctly, a one sided $p$-value is 0.229 + 0.014 = 0.243 and the hypothesis is not rejected.  The null hypothesis would only be rejected if she had guessed all 4 correctly, with $p$-value 0.014.  In a dataset this small, the null hypothesis would be rejected much less than 5\% of the time.
...
%JV: Perhaps important here to also highlight that only reporting RR without baseline risk can also be misleading, if in fact risk in both groups is small.
